...
if($ree){
    mysqli_close($link);
    header("Location:forgot_password.php?send=we send the password by your email please check your email");
}
else{ echo "<h1>we have error</h1>";}
}
if($num==0){
    mysqli_close($link);
    header("Location:forgot_password.php?err=you do not have account");
}
